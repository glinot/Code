...
var currentDate = moment().weekday(0);// default date : current week
    ...
    switch (st) {
        ...
        case  stats_wide.month :
            if (currentDate.month() == moment().month() && currentDate.year() == moment().year()) {
                ...
                return currentDate.endOf("month").date() - 1;
            }
            break;
...
function requestFeature(path, onDone, onFail) {
    $.get(path + "/"+ api_wide_label[currentState]+"/" +localStorage.sportman_id, {
...
function updateView() {
    switch (currentState) {
        case  stats_wide.year :
            $(".text-state").text(currentDate.year());
            break;
        case  stats_wide.month :
            $(".text-state").text(currentDate.date(1).format("MMMM YYYY"));
            break;
        case  stats_wide.week :
            $(".text-state").text(currentDate.weekday(0).format("Do MMMM YYYY") + " - " + currentDate.weekday(6).format("Do MMMM YYYY"));
            break;
    }
}
// default
...
$(".selector-wide").on("changed", function () {
    ...
    console.warn(currentDate.toDate().toLocaleDateString());
    updateStats();
});
...
function buildLabels() {
    switch (currentState) {
        case  stats_wide.year :
            ...
            break;
        case  stats_wide.month :
            return genList(currentDate.endOf("month").date());
            break;
        case  stats_wide.week :
...
function getHealthCurves() {
    ...
    health_ressources.forEach(function (item) {
        requestFeature("/api/stats/" + item, function (dataG) {
            ...
            var data = dataG.map(function(i){return i== null ?  0 : i ;});
...
function fillOverviewBox(elem, data) {
    ...
    $(elem).find(".overview-training-duree").text(data.duree);
...
var today = moment();
$.get("/api/stats/trainings/overview/week/"+localStorage.sportman_id, {year: today.year(), weeknum: today.isoWeek()}, function (data) {
    fillOverviewBox(".week-overview", data);
});
$.get("/api/stats/trainings/overview/month/"+localStorage.sportman_id, {year: today.year(), month: today.month()}, function (data) {
    fillOverviewBox(".month-overview", data);
});
$.get("/api/stats/trainings/overview/year/"+localStorage.sportman_id, {year: today.year()}, function (data) {
...
function fillGlobalTraining(elem, data, state) {
    ...
    switch (state) {
        case  stats_wide.year :
            ...
            break;
        case  stats_wide.month :
            labels = genList(currentDate.endOf("month").date());
            break;
        case  stats_wide.week :
...
$.get("/api/stats/trainings/global/week/"+localStorage.sportman_id, {year: today.year(), weeknum: today.isoWeek()}, function (data) {
    fillGlobalTraining(".week-stats", data, stats_wide.week);
});
$.get("/api/stats/trainings/global/month/"+localStorage.sportman_id, {year: today.year(), month: today.month()}, function (data) {
    fillGlobalTraining(".month-stats", data, stats_wide.month);
});
$.get("/api/stats/trainings/global/year/"+localStorage.sportman_id, {year: today.year()}, function (data) {
